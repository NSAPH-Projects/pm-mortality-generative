...
import diffusers
from diffusers import AutoencoderKL, DDPMScheduler, StableDiffusionPipeline, UNet2DConditionModel, VQModel, UNet2DModel, LDMPipeline
...
from pipelines.ULDM import ULDMPipeline

from dataloader import washu_dataloader as wu_dl
from utils import fill_nan_with_min
    ...
    for tracker in accelerator.trackers:
        if tracker.name == "tensorboard":
            ...
            tracker.log(
                {
                    "validation": [
                        wandb.Image(image, caption=f"Generated sample {i}")
...
def main(cfg: DictConfig):
    ...
    # Scheduler taken from the stable diffusion repo
    scheduler_config = {
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                encoder_hidden_states = None#text_encoder(batch["input_ids"], return_dict=False)[0]
